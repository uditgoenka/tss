...
pub fn known_command_coverage() -> Vec<CommandCoverage> {
    vec![
        ...
        command(
            ...
            CommandParityStatus::PassthroughCompatible,
            "recognized test runner vocabulary; v0.1.02 preserves native output unless a safe adapter exists",
        ),
        command(
            "js",
            "npm/pnpm/yarn/npx/pnpx/bun/deno/corepack/brew scripts",
            CommandParityStatus::PassthroughCompatible,
            "recognized migration vocabulary; package manager output remains raw in v0.1.02",
        ),
        command(
            "go",
            "go test",
            CommandParityStatus::Optimized,
            "wired v0.1.02 filter preserves compile, vet, failure, and summary lines",
        ),
        command(
            "rust",
            "cargo test/check",
            CommandParityStatus::Optimized,
            "wired v0.1.02 filter preserves compiler diagnostics, failed tests, and summaries",
        ),
        command(
            "python",
            "pytest",
            CommandParityStatus::Optimized,
            "wired v0.1.02 filter preserves collection errors, tracebacks, failures, and summaries",
        ),
        command(
            "python",
            "pip/pipx/uv/uvx/poetry/rye",
            CommandParityStatus::PassthroughCompatible,
            "recognized package-tool vocabulary; install and resolver output remains raw in v0.1.02",
        ),
        command(
            "python",
            "ruff",
            CommandParityStatus::Planned,
            "recognized command family without a v0.1.02 adapter yet",
        ),
        command(
            "dotnet",
            "dotnet test",
...
pub fn issue_class_coverage() -> Vec<IssueClassCoverage> {
    vec![
        ...
        issue_class(
            ...
            "v0.1.02 must not auto-trust project-local executable filters or remote telemetry",
        ),
        issue_class(
